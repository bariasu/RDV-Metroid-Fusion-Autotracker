...
client.SetGameExtraPadding(0,0,88,0)
    ...
    if CURRENT_FRAME % CHECK_INTERVAL == 0 and memory.read_u16_le(GAMEMODE_ADDRESS) == 1 then
        ...
        if CHECK_LOADOUT then
            if BEAMS_VALUE & CHARGE_VALUE == CHARGE_VALUE then
                gui.drawImage(IMAGES.charge, 242, 50, nil, nil, false)
            else
                gui.drawImage(IMAGES.charge_gray, 242, 50, nil, nil, false)
            end

            if BEAMS_VALUE & WIDE_VALUE == WIDE_VALUE then
                gui.drawImage(IMAGES.wide, 259, 50, nil, nil, false)
            else
                gui.drawImage(IMAGES.wide_gray, 259, 50, nil, nil, false)
            end

            if BEAMS_VALUE & PLASMA_VALUE == PLASMA_VALUE then
                gui.drawImage(IMAGES.plasma, 276, 50)
            else
                gui.drawImage(IMAGES.plasma_gray, 276, 50)
            end

            if BEAMS_VALUE & WAVE_VALUE == WAVE_VALUE then
                gui.drawImage(IMAGES.wave, 293, 50, nil, nil, false)
            else
                gui.drawImage(IMAGES.wave_gray, 293, 50, nil, nil, false)
            end

            if BEAMS_VALUE & ICE_VALUE == ICE_VALUE then
                gui.drawImage(IMAGES.icebeam, 310, 50, nil, nil, false)
            else
                gui.drawImage(IMAGES.icebeam_gray, 310, 50, nil, nil, false)
            end

            if WEAPONS_VALUE & BASE_MISSILE_VALUE == BASE_MISSILE_VALUE then
                gui.drawImage(IMAGES.missiles, 250, 68, nil, nil, false)
            else
                gui.drawImage(IMAGES.missiles_gray, 250, 68, nil, nil, false)
            end

            if WEAPONS_VALUE & SUPER_MISSILE_VALUE == SUPER_MISSILE_VALUE then
                gui.drawImage(IMAGES.supers, 266, 68)
            else
                gui.drawImage(IMAGES.supers_gray, 266, 68, nil, nil, false)
            end

            if WEAPONS_VALUE & ICE_MISSILE_VALUE == ICE_MISSILE_VALUE then
                gui.drawImage(IMAGES.icemissiles, 283, 68, nil, nil, false)
            else
                gui.drawImage(IMAGES.icemissiles_gray, 283, 68, nil, nil, false)
            end

            if WEAPONS_VALUE & DIFFUSION_MISSILE_VALUE == DIFFUSION_MISSILE_VALUE then
                gui.drawImage(IMAGES.diffusion, 300, 68, nil, nil, false)
            else
                gui.drawImage(IMAGES.diffusion_gray, 300, 68, nil, nil, false)
            end

            if MOVEMENT_VALUE & MORPH_BALL_VALUE == MORPH_BALL_VALUE then
                gui.drawImage(IMAGES.morph, 242, 86, nil, nil, false)
            else
                gui.drawImage(IMAGES.morph_gray, 242, 86, nil, nil, false)
            end

            if WEAPONS_VALUE & BOMBS_VALUE == BOMBS_VALUE then
                gui.drawImage(IMAGES.bombs, 259, 86, nil, nil, false)
            else
                gui.drawImage(IMAGES.bombs_gray, 259, 86, nil, nil, false)
            end

            if WEAPONS_VALUE & POWER_BOMBS_VALUE == POWER_BOMBS_VALUE then
                gui.drawImage(IMAGES.powerbombs, 276, 86, nil, nil, false)
            else
                gui.drawImage(IMAGES.powerbombs_gray, 276, 86, nil, nil, false)
            end

            if MOVEMENT_VALUE & VARIA_VALUE == VARIA_VALUE then
                gui.drawImage(IMAGES.varia, 293, 86, nil, nil, false)
            else
                gui.drawImage(IMAGES.varia_gray, 293, 86, nil, nil, false)
            end

            if MOVEMENT_VALUE & GRAVITY_VALUE == GRAVITY_VALUE then
                gui.drawImage(IMAGES.gravity, 310, 86, nil, nil, false)
            else
                gui.drawImage(IMAGES.gravity_gray, 310, 86, nil, nil, false)
            end

            if MOVEMENT_VALUE & HIGH_JUMP_VALUE == HIGH_JUMP_VALUE then
                gui.drawImage(IMAGES.highjump, 249, 104, nil, nil, false)
            else
                gui.drawImage(IMAGES.highjump_gray, 249, 104, nil, nil, false)
            end

            if MOVEMENT_VALUE & SPEEDBOOSTER_VALUE == SPEEDBOOSTER_VALUE then
                gui.drawImage(IMAGES.speed, 266, 104, nil, nil, false)
            else
                gui.drawImage(IMAGES.speed_gray, 266, 104, nil, nil, false)
            end

            if MOVEMENT_VALUE & SPACE_JUMP_VALUE == SPACE_JUMP_VALUE then
                gui.drawImage(IMAGES.spacejump, 283, 104, nil, nil, false)
            else
                gui.drawImage(IMAGES.spacejump_gray, 283, 104, nil, nil, false)
            end

            if MOVEMENT_VALUE & SCREW_ATTACK_VALUE == SCREW_ATTACK_VALUE then
                gui.drawImage(IMAGES.screwattack, 300, 104, nil, nil, false)
            else
                gui.drawImage(IMAGES.screwattack_gray, 300, 104, nil, nil, false)
            end

            if SECURITY_VALUE & GRAY_VALUE == GRAY_VALUE then
                gui.drawImage(IMAGES.l0, 242, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l0_gray, 242, 122, nil, nil, false)
            end

            if SECURITY_VALUE & BLUE_VALUE == BLUE_VALUE then
                gui.drawImage(IMAGES.l1, 259, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l1_gray, 259, 122, nil, nil, false)
            end

            if SECURITY_VALUE & GREEN_VALUE == GREEN_VALUE then
                gui.drawImage(IMAGES.l2, 276, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l2_gray, 276, 122, nil, nil, false)
            end

            if SECURITY_VALUE & YELLOW_VALUE == YELLOW_VALUE then
                gui.drawImage(IMAGES.l3, 293, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l3_gray, 293, 122, nil, nil, false)
            end

            if SECURITY_VALUE & RED_VALUE == RED_VALUE then
                gui.drawImage(IMAGES.l4, 310, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l4_gray, 310, 122, nil, nil, false)
            end

            gui.drawImage(IMAGES.metroid, 260, 140, nil, nil, false)
            gui.drawString(277, 142, " " .. METROIDS_OBTAINED_VALUE .. "/" .. METROIDS_REQUIRED_VALUE,"white")
