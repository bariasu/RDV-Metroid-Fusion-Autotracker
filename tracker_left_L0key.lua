    ...
    if CURRENT_FRAME % CHECK_INTERVAL == 0 and memory.read_u16_le(GAMEMODE_ADDRESS) == 1 then
        ...
        if CHECK_LOADOUT then
            ...
            if SECURITY_VALUE & GRAY_VALUE == GRAY_VALUE then
                gui.drawImage(IMAGES.l0, 2, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l0_gray, 2, 122, nil, nil, false)
            end

            if SECURITY_VALUE & BLUE_VALUE == BLUE_VALUE then
                gui.drawImage(IMAGES.l1, 19, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l1_gray, 19, 122, nil, nil, false)
            end

            if SECURITY_VALUE & GREEN_VALUE == GREEN_VALUE then
                gui.drawImage(IMAGES.l2, 36, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l2_gray, 36, 122, nil, nil, false)
            end

            if SECURITY_VALUE & YELLOW_VALUE == YELLOW_VALUE then
                gui.drawImage(IMAGES.l3, 53, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l3_gray, 53, 122, nil, nil, false)
            end

            if SECURITY_VALUE & RED_VALUE == RED_VALUE then
                gui.drawImage(IMAGES.l4, 70, 122, nil, nil, false)
            else
                gui.drawImage(IMAGES.l4_gray, 70, 122, nil, nil, false)
            end
